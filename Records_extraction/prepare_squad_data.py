...
def read_samples(context_file, result_file, title="sanguozhi"):
    res = dict()
    res["title"] = title
    res["paragraphs"] = []
...
def training_example():
    ...
    with open("english_speaker_squad.json", "w") as fout:
...
def test_example():
    ...
    res["title"] = "sanguozhi"
    res["paragraphs"] = []
    from sanguozhi import talks as contexts
    for i in range(len(contexts)):
        ...
        res["paragraphs"].append(para_entry)
    out = {"data":[res], "version":"english_squad_v1.0"}
    with open("english_speaker_squad_valid.json", "w") as fout:
        fout.write(json.dumps(out, ensure_ascii=False))
